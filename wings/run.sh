...
# ==================================================
#  Erymont CONTROL PANEL v3.0 | Auto-Detect System
# ==================================================
...
header() {
    ...
    # Title Section
    echo -e "${B} ║${W}${BOLD}   ⚡ Erymont CONTROL PANEL v3.5 ${GR}::${C} SERVER AUTOMATION      ${B}║${N}"
...
show_menu() {
    ...
    echo -e "  ${B}[2]${N} ${G}Install Wings        ${GR}:: (Erymont Script)${N}"
...
while true; do
    ...
    case $opt in
        1) ssl_setup ;;
        2) bash "$SCRIPT_DIR/install.sh" ;;
        3) bash "$SCRIPT_DIR/mang.sh" ;;
        4) bash "$SCRIPT_DIR/db.sh" ;;
        5) uninstall_menu ;;
        0) 
           echo -e "\n${G}👋 Goodbye!${N}"
           exit 0 
